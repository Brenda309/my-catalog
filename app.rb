...
class App
  ...
  def add_books_list
    ...
    update(label_title, label_color, genre, author_first_name, author_last_name)
  ...
  def books_list
    ...
    @books.each_with_index do |list, index|
      puts "
      #{index + 1}) Genre: #{list.genre}, Author: #{list.author}, Source: #{list.source}
      Label: #{list.label}, Publish date: #{list.publish_date}
      Publisher: #{list.publisher}, Cover state: #{list.cover_state}"
    end
    puts "\n"
  ...
  def update(label_title, label_color, genre, author_first_name, author_last_name)
    actual_authors = []
    actual_genres = []
    actual_label = []
    label = "#{label_title} - #{label_color}"
    @label.each { |lab| actual_label.push("#{lab.title} - #{lab.color}") }
